...
<body>
    {{-- Notifikasi (WAJIB ADA) --}}
    @if(session('success'))
        {{-- PERBAIKAN: Menggunakan @json untuk mencegah error Syntax --}}
        <script>Swal.fire('Berhasil', @json(session('success')), 'success');</script>
    @endif
    @if(session('error') || $errors->any())
        {{-- PERBAIKAN: Menggunakan @json untuk mencegah error Syntax --}}
        <script>Swal.fire('Gagal', @json(session('error') ?? $errors->first()), 'error');</script>
    @endif

    <div class="card-body p-0">
        {{-- Kondisi untuk menampilkan error 'Hayoooo' --}}
        @if (count($errors) > 0 && !$errors->has('name') && !$errors->has('email') && !$errors->has('no_telepon') && !$errors->has('tanggal_lahir') && !$errors->has('alamat'))
        <script>
            Swal.fire({
                icon: "error",
                title: "Hayoooo",
...
                    {{-- FORM DINAMIS (BERDASARKAN SOURCE CODE ASLI ANDA) --}}
                    <form class="settings-form" method="POST" action="{{ route('pelanggan.pengaturan.update') }}">
                        @csrf 
                        <div class="form-row">
                            <div class="form-group">
                                <label class="form-label">Nama Lengkap</label>
                                {{-- Menggunakan $user dari controller --}}
...
                                <input type="tel" name="no_telepon" value="{{ old('no_telepon', $user->no_telepon) }}" class="form-input" placeholder="+62 812...">
                                @error('no_telepon') <small style="color: red;">{{ $message }}</small> @enderror
                            </div>
                            <div class="form-group">
                                <label class="form-label">Tanggal Lahir</label>
                                <input type="date" name="tanggal_lahir" value="{{ old('tanggal_lahir', $user->tanggal_lahir) }}" class="form-input">
...
                            <textarea rows="3" name="alamat" class="form-input" placeholder="Jl. Mangga No. 1...">{{ old('alamat', $user->alamat) }}</textarea>
